    ...
    <div>
      ...
      {guests.length === 0 ? (
        ...
        <ul>
          {guests.map(guest => (
            <li key={guest.id}>
              ID: {guest.id} - Created: {new Date(guest.created_at).toLocaleString()}
              <button onClick={() => handleGetGuest(guest.id)}>View</button>
              <button onClick={() => handleUpdateGuest(guest.id)}>Update Activity</button>
      ...
      {selectedGuest && (
        <div>
          ...
          <p>Created At: {new Date(selectedGuest.created_at).toLocaleString()}</p>
          <p>Last Active: {new Date(selectedGuest.last_active_date).toLocaleString()}</p>
          <p>Expires: {new Date(selectedGuest.expiration_date).toLocaleString()}</p>
        </div>
      )}
    </div>
  );
