...
from services.tab_generation_service import (
    create_tab_generation,
    get_all_tab_generations_of_a_tab,
    get_all_tab_generations_of_user,
    get_tab_generation,
    get_tab_generation_1st,
    verify_user_exists,
    verify_tab_exists,
)
...
@router.post(
    "/users/{user_id}/tabs/{tab_id}/tab_generations/",
    response_model=TabGenerationResponse,
)
async def create_new_tab_generation(
    ...
    return await create_tab_generation(
        user_id, tab_id, tab_generation_create, db, background_tasks
    )


@router.get(
    "/users/{user_id}/tab_generations/", response_model=List[TabGenerationResponse]
)
def read_all_tab_generations_of_user(user_id: int, db: Session = Depends(get_db)):
    try:
...
@router.get(
    "/users/{user_id}/tabs/{tab_id}/tab_generations/",
    response_model=List[TabGenerationResponse],
)
...
@router.get(
    "/users/{user_id}/tabs/{tab_id}/tab_generations/1st",
    response_model=TabGenerationResponse,
)
def read_tab_generation_1st(user_id: int, tab_id: int, db: Session = Depends(get_db)):
    try:
        ...
        if tab_generation is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="TabGeneration not found"
            )
...
@router.get(
    "/users/{user_id}/tabs/{tab_id}/tab_generations/{tab_generation_id}",
    response_model=Optional[TabGenerationResponse],
)
def read_tab_generation(
    user_id: int, tab_id: int, tab_generation_id: int, db: Session = Depends(get_db)
):
    try:
        # Get tab generation and handle possible exceptions
